...
def _load_table(path: str) -> Dict[str, int]:
    ...
    global index_to_cat#a bit dirty hack to retrieve gram categories
    index_to_cat = {i: cat for (i, cat) in enumerate(spec["ordering"])}
    table: Dict[str, int] = {}
        ...
        for (n, (lemma, inflection, features_str)) in enumerate(tsv_reader, 1):
            ...
            for feature in features:
                ...
                if current:
                    ...
                    exit(1)
                slots[index] = index_to_cat[index].replace("Part of Speech","POS").replace(" ","-")+"="+feature#adding gram categories to the output
