...
import numpy as np
import random
from . import prepare_sigmorphon_data
import progressbar
import datetime
import time
import os
...
def main(train_path, dev_path, test_path, results_file_path, sigmorphon_root_dir, input_dim, hidden_dim, feat_input_dim,
    ...
    for marker in [str(i) for i in range(MAX_PREDICTION_LEN)]:
...
def train_model(model, char_lookup, feat_lookup, R, bias, encoder_frnn, encoder_rrnn, decoder_rnn, W_c, W__a, U__a,
    ...
    for e in range(epochs):
        ...
        train_set = list(zip(train_lemmas, train_feat_dicts, train_words))
...
def bilstm_transduce(encoder_frnn, encoder_rrnn, lemma_char_vecs):
    ...
    # BiLTSM outputs
    blstm_outputs = []
    for i in range(len(lemma_char_vecs)):
